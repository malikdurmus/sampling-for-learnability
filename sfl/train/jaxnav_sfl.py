...
def main(config):
    ...
    def get_learnability_set(rng, network_params):
        ...
        def _batch_step(unused, rng):
            ...
            # sample envs for learnability
            #------Reset the envs START
            rng, _rng = jax.random.split(rng)
            ...
            runner_state, traj_batch = jax.lax.scan( # traj_batch has [num_step] [num_envs]
                                                     # traj_batch.reward[60][75] gives you the reward in
                                                     # 60th step of the 75.th environment
                _env_step, runner_state, None, config["ROLLOUT_STEPS"] # takes 1000 steps for each environment
            )

            ##jax.debug.breakpoint()
            print('traj batch done', traj_batch.done.shape) # runs once
            print('traj batch info', traj_batch.info["NumC"].shape)
            # done_by_env has shape [step][agent][environment]
            ...
            reward_by_env = traj_batch.reward.reshape((-1, env.num_agents, config["BATCH_SIZE"]))
                                                    # traj_batch.info["Return"] = [step] [env] [1]
            info_by_actor = jax.tree_util.tree_map(lambda x: x.swapaxes(2, 1).reshape((-1, BATCH_ACTORS)), traj_batch.info)
                                                    # swap axes gives: [step (here 1000)][agents (here 1)][envs (here 1000)]
                                                    # reshaping gives: [1000][1000]
            print('done_by_env', done_by_env.shape)
            print('reward_by_env', reward_by_env.shape)
            print('info_by_actor', info_by_actor)
            # _calc_outcomes_by_agent is a vmapped function. each vmap runner gets an environment (slice along axis 1 in decorator)
        ...
        #jax.debug.breakpoint()
        _, (learnability, env_instances) = jax.lax.scan(_batch_step, None, rngs, config["NUM_BATCHES"]) # 5
        ##jax.debug.breakpoint()
